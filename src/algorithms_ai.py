...
class AStar:
    ...
    def _create_graph_from_points(self, points: pd.DataFrame, use_3d: bool = False) -> nx.Graph:
        ...
        G = nx.Graph()
        points_dict = points.to_dict('index')
        
        # Add nodes with coordinates directly from DataFrame
        for idx, point_data in points_dict.items():
            G.add_node(idx, 
    ...
    def merge_subsets(self, start: pd.DataFrame, end: pd.DataFrame) -> pd.DataFrame:
        ...
        for subset in self.subsets_cache:
            ...
            # A* pathfinding with direct coordinate access
            path = nx.astar_path(
                G, 
                subset.shape[0] - 2,
                subset.shape[0] - 1,
